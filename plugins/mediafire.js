...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  if (!args[0])
    return conn.reply(
      m.chat,
      `『 ARSLAN-X 』\n\n🔍 *Please provide a valid MediaFire link!*\n\n📌 *Example:* ${usedPrefix + command} https://www.mediafire.com/file/xxxxx`,
      m
    )
  ...
  try {
    ...
    if (!items.length)
      return conn.reply(m.chat, `『 ARSLAN-X 』\n\n❌ *No files found on this link.*`, m)

    for (const item of items) {
      const direct = await getDirectDownload(item.filePageUrl)
      if (!direct) {
        await conn.reply(m.chat, `『 ARSLAN-X 』\n\n❌ *Failed to fetch:* ${item.filename}`, m)
        continue
      }
...
╰──────────────────• 乂`.trim()
    ...
    conn.reply(m.chat, `『 ERROR 乂 SYSTEM 』\n\n❌ ${e.message}`, m)
  }
}
